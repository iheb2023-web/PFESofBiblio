...
class DemandeEmpruntPage extends StatelessWidget {
  final BorrowController _borrowController = Get.find<BorrowController>();

  DemandeEmpruntPage({Key? key}) : super(key: key);
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      _buildBorrowStatus(borrow.borrowStatus?.toString().split('.').last ?? 'UNKNOWN'),
                    ],
                  ),
                ),
              ],
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Mes demandes d\'emprunt'),
        elevation: 0,
      ),
      body: RefreshIndicator(
        onRefresh: () => _borrowController.loadAllBorrows(),
        child: Obx(() {
